...
category_revenue.coalesce(1).write.mode('overwrite').parquet("s3a://test-bucket/gold/category_revenue")
...
brand_revenue = purchase_df.groupBy('brand') \
    ...
    .orderBy(sf.desc('revenue'))
brand_revenue.coalesce(1).write.mode('overwrite').parquet("s3a://test-bucket/gold/brand_revenue")

# 3. 일별(event_date) 추이
daily_events = df.groupBy('event_date', 'event_type') \
    .count() \
    .orderBy('event_date', 'event_type')
daily_events.coalesce(1).write.mode('overwrite').parquet("s3a://test-bucket/gold/daily_events")
...
daily_purchase = purchase_df.groupBy('event_date') \
    ...
    .orderBy('event_date')
daily_purchase.coalesce(1).write.mode('overwrite').parquet("s3a://test-bucket/gold/daily_purchase")

# 4. 가격 분포 / 기술통계
price_stats_df = df.select('price').describe()
price_stats_df.coalesce(1).write.mode('overwrite').parquet("s3a://test-bucket/gold/price_stats")
...
price_percentiles_df.coalesce(1).write.mode('overwrite').parquet("s3a://test-bucket/gold/price_percentiles")

purchase_price_stats_df = purchase_df.select('price').describe()
purchase_price_stats_df.coalesce(1).write.mode('overwrite').parquet("s3a://test-bucket/gold/purchase_price_stats")
